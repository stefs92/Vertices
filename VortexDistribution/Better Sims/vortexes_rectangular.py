...
class vortex_dist:
    ...
    def doubl(self):
        ...
        self.res = 2*self.res
        self.allocate_arrays()
        for i in range(2*prevres):
            for j in range(prevres):
                self.v_phi[2*i,2*j] = self.tmp_whtvr[i,j]
                if i < 2*prevres-1:
                    self.v_phi[2*i+1,2*j] = 0.5*(self.tmp_whtvr[i,j] + self.tmp_whtvr[i+1,j])
                if j < prevres-1:
                    self.v_phi[2*i,2*j+1] = 0.5*(self.tmp_whtvr[i,j] + self.tmp_whtvr[i,j+1])
                if (i < 2*prevres-1) and (j < prevres-1):
                    self.v_phi[2*i+1,2*j+1] = 0.5*(self.v_phi[2*i+1,j] + self.v_phi[2*i,2*j+1])
