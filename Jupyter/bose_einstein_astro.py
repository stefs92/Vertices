...
import power_spec2 as pws
import power_real as pwr
...
hbar = 6.582119514e-16  #in eV but that's fine since only hbar/m appears 
m = (1.0e-22)/light_speed**2 # ditto
hubble_time = (3.154e+7)*(14e+9)
h_bar = 1.0545718e-34
kilo_parsec = 3.086e+19
solar_mass = 2e+30

#now define dimensionless coefficients multiplying potential and kinetic term
potential_coeff = (m/hbar)*(hubble_time/kilo_parsec)*G*solar_mass
kinetic_coeff = (hbar/(2*m))*(hubble_time/kilo_parsec**2)
	...
	for i in range(a.shape[0]):
		...
		for j in range(a.shape[1]):
			...
			for l in range(a.shape[2]):
				...
				a[i,j,l] = -1*a[i,j,l]/(k_sq)

@jit
def update_position(a, t_step, fcc=False):
	for i in range(a.shape[0]):
		i_div = a.shape[0] - i if i > a.shape[0]/2 else i
		for j in range(a.shape[1]):
			j_div = a.shape[0] - j if j > a.shape[0]/2 else j
			for l in range(a.shape[2]):
				...
				if fcc:
					...
					k_sq = i_div**2 + j_div**2 + l_div**2
				a[i,j,l] *= np.exp(-1j*t_step*kinetic_coeff*k_sq)
...
class fluid_sim:
	...
	def get_V(self):
		...
		if not self.gpu:
			...
			self.fourier_grid *= (self.length**2)/np.pi
			...
			self.g_pot_func(self.g_fourier, np.float64(1/(np.pi*self.length)), 
			...
			self.g_V /= -1*self.N	# added a minus sign here because I couldn't pinpoint where you define Laplacian ... - SI
			self.g_V += self.lam*self.g_rho**2

	def update_momentum(self, factor):
		if not self.gpu:
			self.psi *= np.exp(-1j*factor*potential_coeff*self.V)
		else:
			self.g_psi_hat[...] = -1.0j*factor*potential_coeff*self.g_V
	...
	def update_position(self, time_step):
		...
		if not self.gpu:
			...
			update_position(self.psi_hat, time_step*(4*np.pi**2)/(self.length**2))
			...
			factor = np.float64(time_step*kinetic_coeff*4*(np.pi**2)/(self.length**2) )
	...
	def leap_frog_steps(self, steps, t_step):
		...
		for i in range(steps):
			if i%(steps/10) == 0:
				print i
				if self.gpu:
					pwr.power_spectrum2(self.g_psi,self.length)
					ps.plot_phase(self.g_psi)
					ps.plot_slice(self.g_psi)
					ps.plot_potential(self.g_V)
				else:
